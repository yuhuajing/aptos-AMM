module ClayCoin::amm {
    ...
    struct Pair<phantom Asset0Type, phantom Asset1Type> has key {
        ...
        freeze_capability:FreezeCapability<LiquidityCoin<Asset0Type, Asset1Type>>,
    ...
    public fun accept<Asset0Type, Asset1Type>(root: &signer) {
        ...
        let (burn_capability, freeze_capability, mint_capability) = coin::initialize<LiquidityCoin<Asset0Type, Asset1Type>>(
            root,
    ...
    public entry fun mint_script<Asset0Type, Asset1Type>(account: &signer, amount0: u64, amount1: u64) acquires Pair {
        ...
        if (!coin::is_account_registered<LiquidityCoin<Asset0Type, Asset1Type>>(sender)) coin::register<LiquidityCoin<Asset0Type, Asset1Type>>(account);
    ...
    public entry fun burn_script<Asset0Type, Asset1Type>(account: &signer, liquidity: u64) acquires Pair {
        ...
        if (!coin::is_account_registered<Asset0Type>(sender)) coin::register<Asset0Type>(account);
        if (!coin::is_account_registered<Asset1Type>(sender)) coin::register<Asset1Type>(account);
    ...
    public entry fun swap_script<In, Out>(account: &signer, amount_in: u64, amount_out_min: u64) acquires Pair {
        ...
        let sender = signer::address_of(account);
        if (!coin::is_account_registered<Out>(sender)) coin::register<Out>(account);
...
#[view]
    public  fun get_reserves_script<In, Out>(): (u64, u64) acquires Pair {
        get_reserves<In, Out>()
    }
#[view]
    public  fun get_amount_out_script<In, Out>(amount_in: u64): u64 acquires Pair {
        get_amount_out<In, Out>(amount_in)
    }
#[view]
    public  fun get_amount_in_script<In, Out>(amount_out: u64): u64 acquires Pair {
        get_amount_in<In, Out>(amount_out)
    }
#[view]
    public  fun find_pair_script<Asset0Type, Asset1Type>(): u8 {
    ...
    //     // init 2 fake coins
    //    let (burn_cap_a,freeze_cap_a,mint_cap_a)= coin::initialize<FakeMoneyA>(
    //         &root,
    ...
    //         true
    //     );
    //     let (burn_cap_b,freeze_cap_b,mint_cap_b) = coin::initialize<FakeMoneyB>(
    //         &root,
